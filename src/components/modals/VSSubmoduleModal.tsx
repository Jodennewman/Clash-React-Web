    ...
    >
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-2/3">
          <div className="aspect-video relative rounded-[--border-radius-lg] overflow-hidden shadow-[2px_2px_8px_rgba(0,0,0,0.05)] dark:shadow-[0_0_15px_rgba(53,115,128,0.15)]">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-black/20 hover:bg-black/30 transition-colors cursor-pointer group">
              <div className="bg-white dark:bg-[--primary-orange] rounded-full w-16 h-16 flex items-center justify-center shadow-md transform transition-transform group-hover:scale-110">
                <Play className="h-8 w-8 text-[--primary-orange] dark:text-white fill-current ml-1" />
              </div>
            </div>
          ...
          {/* Module description */}
          <div className="mt-6">
            <h3 className="text-[--text-navy] dark:text-white text-xl font-semibold mb-3">About This Module</h3>
            <p className="text-[--text-navy]/80 dark:text-white/70">
            ...
            <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4">
              {/* Module stats */}
              <div className="bg-white/50 dark:bg-white/5 rounded-[--border-radius-md] p-4 flex flex-col items-center text-center">
                <div className="text-[--primary-orange] dark:text-[--primary-orange] text-2xl font-bold mb-1">
                  {submodules.length}
                </div>
                <div className="text-[--text-navy]/70 dark:text-white/60 text-sm">
                  Lessons
                </div>
              </div>
        ...
        <div className="w-full md:w-1/3">
          <div className="bg-white/50 dark:bg-[--bg-navy]/50 rounded-[--border-radius-lg] overflow-hidden border border-[--bg-cream-darker]/30 dark:border-white/5">
            <div className="p-4 border-b border-[--bg-cream-darker]/30 dark:border-white/5">
              <h3 className="text-[--text-navy] dark:text-white font-semibold">Module Content</h3>
            </div>
            ...
            <div className="max-h-[500px] overflow-y-auto">
              <ul className="divide-y divide-[--bg-cream-darker]/30 dark:divide-white/5">
                {submodules.map((submodule, index) => (
                  <li
                    key={submodule.id}
                    className={`p-4 flex items-center hover:bg-[--bg-cream]/30 dark:hover:bg-white/5 cursor-pointer transition-colors ${selectedSubmoduleId === submodule.id ? 'bg-[--bg-cream]/50 dark:bg-white/10' : ''}`}
                    onClick={() => handlePlayModule(submodule.id)}
                  >
                    <div className="flex-shrink-0 mr-3 text-[--primary-orange] dark:text-[--primary-orange]">
                      {submodule.isLocked ? (
                        <Lock className="h-5 w-5" />
                      ) : submodule.isCompleted ? (
                        <CheckCircle className="h-5 w-5" />
                      ) : (
                        <div className="w-5 h-5 rounded-full border-2 border-[--primary-orange] dark:border-[--primary-orange] flex items-center justify-center">
                    ...
                    <div className="flex-grow">
                      <h4 className={`text-sm font-medium ${submodule.isLocked ? 'text-[--text-navy]/50 dark:text-white/40' : 'text-[--text-navy] dark:text-white'}`}>
                        {submodule.title}
                      </h4>
                      <div className="flex items-center mt-1">
                        <span className={`text-xs ${submodule.isLocked ? 'text-[--text-navy]/40 dark:text-white/30' : 'text-[--text-navy]/60 dark:text-white/50'}`}>
                          {submodule.duration}
                        </span>
                        {submodule.isLocked && (
                          <span className="ml-2 text-xs px-2 py-0.5 bg-[--bg-cream-darker]/30 dark:bg-white/10 rounded-full text-[--text-navy]/60 dark:text-white/60">
                            Premium
                          </span>
                        )}
                      </div>
          ...
          <div className="mt-6 flex flex-col gap-3">
            ...
            <button
              className="border border-[--secondary-teal] text-[--secondary-teal]
