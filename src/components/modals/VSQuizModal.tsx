    ...
    >
      {!showResults ? (
        <>
              ...
              {Array(totalSteps).fill(0).map((_, idx) => (
                <div
                  ...
                  className={`h-1.5 w-6 rounded-full transition-all duration-300 ${idx <= currentStep
                    ...
                    : 'bg-[--bg-cream-darker]/30 dark:bg-white/10'
                    }`}
                />
              ))}
            </div>
          ...
          {/* Question */}
          <div className="mb-8">
            <h3 className="text-xl font-semibold mb-6 text-[--text-navy] dark:text-white">
              ...
              {currentQuestion.options.map((option, idx) => (
                <div
                  ...
                  className={`p-4 rounded-[--border-radius-md] border cursor-pointer transition-all ${answers[currentQuestion.id] === option.value
                    ? 'border-[--primary-orange] bg-[--primary-orange]/10 dark:border-[--primary-orange] dark:bg-[--primary-orange]/10'
                    : 'border-[--bg-cream-darker]/30 dark:border-white/10 hover:border-[--bg-cream-darker] dark:hover:border-white/30'
                    }`}
                  onClick={() => handleSelect(option.value)}
                >
                  <div className="flex items-center">
                    <div className={`w-5 h-5 rounded-full border ${answers[currentQuestion.id] === option.value
                      ? 'border-[--primary-orange] bg-[--primary-orange] dark:border-[--primary-orange] dark:bg-[--primary-orange]'
                      : 'border-[--text-navy]/30 dark:border-white/30'
                    ...
                    <span className="text-[--text-navy]/80 dark:text-white/80">{option.label}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {currentQuestion.explanation && answers[currentQuestion.id] && (
              <div className="mt-6 p-4 bg-[--bg-cream]/30 dark:bg-white/5 rounded-[--border-radius-md] border border-[--bg-cream-darker]/20 dark:border-white/5">
        ...
        <div className="text-center">
          <div className="w-32 h-32 mx-auto mb-6 relative">
            <svg viewBox="0 0 36 36" className="w-full h-full transform rotate-[-90deg]">
              ...
              <circle
                ...
                strokeLinecap="round"
                className="text-[--primary-orange] dark:text-[--primary-orange]"
              />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-3xl font-bold text-[--text-navy] dark:text-white">{score}%</span>
            </div>
          </div>

          <h3 className="text-2xl font-bold mb-3 text-[--text-navy] dark:text-white">
            {score >= 80 ? 'Great job!' : score >= 50 ? 'Good effort!' : 'Keep practicing!'}
          </h3>
          
          <p className="text-[--text-navy]/70 dark:text-white/70 mb-8 max-w-md mx-auto">
            {score >= 80
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={handleReset}
              className="px-6 py-2 border border-[--secondary-teal] text-[--secondary-teal] dark:border-white/20 dark:text-white rounded-[--border-radius-md] hover:bg-[--secondary-teal]/5 dark:hover:bg-white/5 transition-all duration-[--transition-bounce] hover:translate-y-[-2px] hover:scale-[1.02]"
            >
              Retake Quiz
            </button>
