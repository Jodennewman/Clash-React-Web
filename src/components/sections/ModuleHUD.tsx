...
const BigSquare = React.forwardRef<HTMLDivElement, SquareProps>(
  ({ section }, ref) => {
    ...
    React.useEffect(() => {
      ...
      const el = ref as React.RefObject<HTMLDivElement | null>;
      ...
      img.onerror = () => {
        if (!mounted.current || !el.current) return;
        console.error(
          `Failed to load BigSquare thumbnail for ${section.id}: ${thumbnailUrl}`,
        );
        ...
        const backupThumbnail =
          "/assets/main/DataBaseThumbnails/renamed/default.webp";
        ...
        backupImg.onload = () => {
          ...
          el.current.style.setProperty(
            "background-image",
            `linear-gradient(135deg, ${section.color}99, ${section.color}cc), url('${backupThumbnail}')`,
            "important",
          );
          el.current.style.setProperty("background-size", "cover", "important");
          el.current.style.setProperty(
    ...
    if (isSystemSection) {
      ...
      selectedSectionModules.forEach((module) => {
        ...
        if (loadState === "success") {
          ...
          const backupThumbnail =
            "../assets/main/DataBaseThumbnails/renamed/the_algorithm.webp";
          ...
          img.onerror = () => {
            console.error(
              `Failed to load thumbnail for module: ${module.id} from ${thumbnailUrl}`,
            );
            ...
            const backupThumbnail =
              "/assets/main/DataBaseThumbnails/renamed/default.webp";
            console.log(
              `Using default fallback thumbnail for ${module.id}: ${backupThumbnail}`,
            );
            ...
            if (moduleLoadState.get("backup-loaded") !== "success") {
              ...
              backupImg.onload = () => {
                ...
                moduleEl.style.setProperty(
                  "background-image",
                  `linear-gradient(135deg, ${sectionColor}99, ${sectionColor}cc), url('${backupThumbnail}')`,
                  "important",
                );
                moduleEl.style.setProperty(
                  "background-size",
              ...
              backupImg.onerror = () => {
                console.error(
                  `Even default thumbnail failed to load: ${backupThumbnail}`,
                );
        ...
        if (module.featured || module.founderMustWatch) {
          ...
          indicatorEl.className =
            "absolute top-2 right-2 w-2 h-2 rounded-full bg-[var(--hud-accent-red)]";
    ...
    <div>
              ...
              <div className="grid grid-cols-1 gap-[calc(var(--square-gap-y)*1.2)]">
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                    <div className="text-white font-bold text-lg mb-2">
                      {courseUtils
                        .getSystemData("engine_room")
                        ?.title?.toUpperCase() || "ENGINE ROOM"}
                    </div>
                    <div className="w-12 h-1 bg-white/30 rounded-full mb-4"></div>
                    <div className="text-white/80 text-sm mb-4">
                      {courseUtils.getSystemData("engine_room")?.subtitle ||
                        "Content Production"}
                    </div>
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                    <div className="text-white font-bold text-lg mb-2">
                      {courseUtils
                        .getSystemData("viral_os")
                        ?.title?.toUpperCase() || "VIDEO OS"}
                    </div>
                    <div className="w-12 h-1 bg-white/30 rounded-full mb-4"></div>
                    <div className="text-white/80 text-sm mb-3">
                      {courseUtils.getSystemData("viral_os")?.subtitle ||
                        "Editing Tools"}
                    </div>
      ...
      {selectedModuleId && (
        <VSSubmoduleModal
          ...
          submodules={courseUtils
            .getSubmodulesForModule(selectedModuleId)
            .map((submodule) => ({
              id: submodule.id,
              title: submodule.title,
              duration:
                submodule.formattedDuration || `${submodule.duration}:00`,
